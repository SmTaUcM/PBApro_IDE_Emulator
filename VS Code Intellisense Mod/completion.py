...
class JediCompletion(object):
    ...
    def _serialize_tooltip(self, definitions, identifier=None, termFound=False):
    ...
    def writeToFile(self, data, sMessage=None):
        '''Debug method used to output script information to a dext file.'''

        with open(r"C:\Users\smtau\Desktop\New folder\dump.txt", "a") as tgtFile:
            if sMessage:
                tgtFile.write(sMessage + " " + str(data) + "\n")
            else:
                tgtFile.write(str(data) + "\n")
        #--------------------------------------------------------------------------------------------------------------------------------------------#
...
# Added code to inject VS Code intellisense for PbaProObjects().
#------------------------------------------------------------
                pbaData = self.handlePbaData(rq)
                if pbaData:
                    response = pbaData
#------------------------------------------------------------
    ...
    def handlePbaData(self, initRequest):
        '''Method that injects intellisense for PbaProObject'''

        response = False
        initRequest = json.loads(initRequest) # Converts the request from a dictionary represented as a string into a real disctionary.
        lookup = initRequest["lookup"]
        id = initRequest["id"]
        source = initRequest.get("source", None),
        line = initRequest["line"] + 1,
        column = initRequest["column"],
        path = initRequest["path"]
        definition = False

        # Determine the word of text at which the user's cursor has stopped.
        completionWords = str(source[0].split("\r\n")[line[0] - 1][ : column[0]]).split(" ")[ -1 ].split(".")

        # Find 'completionWord = ' definition.
        for line in source[0].split("\r\n"):
            if completionWords[0] + " =" in line or completionWords[0] + " =" in line:
                definition = line

        className = self.getPbaClass(path, definition)

        if not definition:
            return response

        # Re-write the suggestions and send them to VS Code.
        if "= PbaProObject" in definition or "=PbaProObject" in definition:

            #--------------------------------------------------------------------------------------------------------------
            if lookup == "completions":
                return json.dumps({"id": id, "results": pbaDefinitions[className][lookup]})
            #--------------------------------------------------------------------------------------------------------------

            #--------------------------------------------------------------------------------------------------------------
            elif lookup == "tooltip":

                    return json.dumps({"id": id, "results": pbaDefinitions[className][lookup][completionWords[-1]]})
            #--------------------------------------------------------------------------------------------------------------

            #--------------------------------------------------------------------------------------------------------------
            elif lookup == "arguments":

                index = len(completionWords[-1].split(",")) - 1
                argData = pbaDefinitions[className][lookup][completionWords[-1].split("(")[0]]
                response = {"id": id,
				            "results": [
						                {"raw_docstring": argData[0]["raw_docstring"],
							             "name": argData[0]["name"],
				                         "docstring": argData[0]["docstring"],
				                         "params": argData[0]["params"],
        								 "paramindex": index,
        								 "bracketstart": [index],
        								 "description": argData[0]["description"]
                                         }
                                        ]
                            }
    ...
    def getPbaClass(self, path, definition):

        if definition:
            definitionText = definition.split('"')[-2]
            self.objectList = []
            self.parseXMLFiles(path)

            if definitionText == "PBApro":
                return "PProApp"

            elif 'ResourceList.MIL-Board' == definitionText[ : -1]:
                return "MilResource"

            else:
                for item in self.objectList:
                    if item != None:
                        if definition.split('"')[-2] == item[0]:
                            return item[1]
        return None
        #--------------------------------------------------------------------------------------------------------------------------------------------#


    def parseXMLFiles(self, path):

        xmlDicts = []
        # Discover PBA.Pro related files and search for objectPath within them.
        for tgtFile in self.__detectPbaFiles(path):
        ...
        for xmlDict in xmlDicts:
            self.__searchForCL(xmlDict["PPU"])
    #------------------------------------------------------------------------------------------------------------------------------------------------#


    def __detectPbaFiles(self, path):
        ...
        for dirpath, dirnames, filenames in os.walk(newCwd):
            for pbaFile in filenames:
                if (".ppro" in pbaFile or ".tmpl" in pbaFile) and ".bak" not in pbaFile:
                    pbaFiles.append((dirpath + "\\" + pbaFile).replace("\\\\", "\\"))

        return pbaFiles
    #------------------------------------------------------------------------------------------------------------------------------------------------#


    def __searchForCL(self, xmlDict, parentObj=None):
        ...
        if str(type(xmlDict.get("CL"))) == "<type 'list'>":
            for item in xmlDict.get("CL"):
                objectName = item["@n"]
                className = item["@ty"]
                newObj = self.__createObject(className, objectName)
                newObj2 = self.__addParentInfo(parentObj, newObj)

                # Detect further child assets.
                if "CL" in item.keys():
                    self.__searchForCL(item, parentObj=newObj)

                self.objectList.append(newObj2)
        ...
        # If our XML class has no child classes.
            ...
            if "CL" in xmlDict["CL"].keys():
                self.__searchForCL(xmlDict["CL"], parentObj=newObj)

            self.objectList.append(newObj2)
    #------------------------------------------------------------------------------------------------------------------------------------------------#


    def __addParentInfo(self, parentObj, newObj):
        ...
        if parentObj != None:
            ...
            obj[0] = parentObj[0] + "." + newObj[0]
            return obj
    #------------------------------------------------------------------------------------------------------------------------------------------------#


    def __createObject(self, className, objectName, properties=None, parentObj=None):
